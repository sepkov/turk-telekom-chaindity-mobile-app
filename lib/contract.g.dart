...
class Contract extends _i1.GeneratedContract {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> Edit(String uri, BigInt tokenId,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '15b6bd3a'));
    final params = [uri, tokenId];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> MintId(String uri,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[2];
    assert(checkSignature(function, 'dab28d57'));
    final params = [uri];
    return write(credentials, transaction, function, params);
  }
